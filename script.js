//dont look at this its terrible please dont flame me :skull:
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    findButton.addEventListener('click', handleFindElement);

    //added enter sensitivity
    ...
    function handleFindElement() {
        ...
        if (symbol === '') {
            resultDiv.textContent = 'Please enter an element symbol.';
        ...
        if (element) {
            resultDiv.innerHTML = `<strong>${element.name}</strong> (symbol: ${symbol}) is at group ${element.group} and period ${element.period} on the periodic table.`;
            resultDiv.classList.add('success');
        } else {
            resultDiv.textContent = `Element with symbol '${symbol}' not found in the periodic table. Please try again :) `;
            resultDiv.classList.add('error');
        }
    }
